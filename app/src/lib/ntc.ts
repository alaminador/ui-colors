// Name That Color — data and matching from chir.ag/projects/ntc (Chirag Mehta, CC BY 2.5)
...
export function nameColor(hex: string): string {
  ...
  const [h, s, l] = hslOf(clean);
  let cl = -1;
  let df = -1;
  for (let i = 0; i < names.length; i += 1) {
    if (clean === names[i][0]) return names[i][1];
    const [nr, ng, nb] = rgbOf(names[i][0]);
    const [nh, ns, nl] = hslOf(names[i][0]);
    const ndf1 = (r - nr) ** 2 + (g - ng) ** 2 + (b - nb) ** 2;
    const ndf2 = (h - nh) ** 2 + (s - ns) ** 2 + (l - nl) ** 2;
  ...
  return cl < 0 ? "Unknown" : names[cl][1];
}
